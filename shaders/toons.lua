...
vec4 get_pixel(sampler2D tex0, vec2 coords, float dx, float dy) 
{
 return Texel(tex0,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(sampler2D tex0, in vec2 coords)
{
  float dxtex = 1.0 / float(textureDimensions.x);
  float dytex = 1.0 / float(textureDimensions.y);
  ...
  for (int i=-1; i<2; i++) {
   for(int j=-1; j<2; j++) {
    k++;
    pix[k] = avg_intensity(get_pixel(tex0, coords,float(i)*dxtex,
                                          float(j)*dytex));
   }
  }
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
  ...
  vec4 tc = vec4(1.0, 0.0, 0.0, 1.0);
    ...
    vec3 colorOrg = Texel(texture, uv).rgb;
    vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
    vHSV.x = nearestLevel(vHSV.x, 0);
    vHSV.y = nearestLevel(vHSV.y, 1);
    vHSV.z = nearestLevel(vHSV.z, 2);
    float edg = IsEdge(texture, uv);
